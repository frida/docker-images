 ...
 * Data is received from the protocol module in struct packet_control's.
 *
...
#define TRAP_FILE                   "/etc/system/trap/devi-control-opts"
...
//static void calculate_cursor_move (struct data *dp, struct packet_control *p);

input_module_t control_filter = {

	NULL,
	NULL,
	NULL,
	0,
	DEVI_CLASS_CONTROL | DEVI_MODULE_TYPE_FILTER,
	"ctrl",
	__DATE__,
	"xy",
...
{
	struct packet_control *p = (struct packet_control *) arg;
	//struct data *dp = module->data;
	static struct packet_control mbuf;
	//int valid = 0;
	//input_module_t *pkbd;
	...
	for (; num; num--) {
		if (verbosity >= 3)
			printf ("Raw control filter data Rx: %d Ry: %d Rz: %d Buttons: %llu\n", p->Rx, p->Ry, p->Rz, p->button_state);
...
__SRCVERSION( "$URL: http://svn/product/branches/6.5.0/trunk/hardware/devi/lib/control_filter.c $ $Rev: 583544 $" );
