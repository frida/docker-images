 ...
 * Description
 *
 * Use this for older Intel flash such as the "J" model StrataFlash that can
 * store the block lock status accross reboots.
 */

int f3s_iCFI_v2unlockall(f3s_dbase_t *dbase,
                         f3s_access_t *access,
                         uint32_t flags,
                         uint32_t offset)
{
	...
	send_command(memory, INTEL_UNLOCKALL);
...
__SRCVERSION("$URL: http://svn/product/branches/6.5.0/trunk/hardware/flash/mtd-flash/intel/iCFI_v2unlockall.c $ $Rev: 710521 $")
#endif
