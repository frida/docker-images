...
int f3s_aCFI_v2suspend(f3s_dbase_t *dbase,
                       f3s_access_t *access,
                       uint32_t flags,
                       uint32_t offset)
{
	...
	/* Wait for 5us, per Spansion(5us typ. 20us max) */
	nanospin_ns(5000);

	/* Wait until DQ6 stops toggling; Recommended value:100us. 
	 * Since the real loop is much slower than empty loop it was
	 * caliberated, the real delay should be longer.
	 */
	loops = UINT32_MAX;
	if (flashcfg.erase_susp_timeout != 0) {
		loops = nanospin_ns_to_count(flashcfg.erase_susp_timeout*1000);
	}
	while (loops--) {
	...
	/* Shouldn't get here, erase suspend timeout? */
	send_command(memory, AMD_ERASE_RESUME);
	delay(5);
	fprintf(stderr, "%s: %d Erase-suspend tiemout \n", __func__, __LINE__);
	return (EIO);
}
...
__SRCVERSION("$URL: http://svn/product/branches/6.5.0/trunk/hardware/flash/mtd-flash/amd/aCFI_v2suspend.c $ $Rev: 710521 $")
#endif
