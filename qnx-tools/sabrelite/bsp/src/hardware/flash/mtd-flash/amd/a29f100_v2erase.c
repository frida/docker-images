 ...
 * Description
 *
 * This is the most generic MTDv2 erase callout.
 */

int f3s_a29f100_v2erase(f3s_dbase_t * dbase,
                        f3s_access_t * access,
                        uint32_t flags,
                        uint32_t offset)
{
	...
	uintptr_t			amd_cmd2;
#if 0 
	F3S_BASETYPE		mask3 = (1 << 3) * flashcfg.device_mult;
	...
	nanospin_ns(6000); // workaround for DQ5 error. delay is not necessary according to Spansion.

#if 0 // Remove DQ6/DQ3 polling, which is not neccessary according to Spansion
	/* If DQ6 is toggling the command was accepted */
	do {
	 	status  = readmem(memory); 
	 	status2 = readmem(memory); 
	} while (((status ^ status2) & mask6) != mask6);

	/* Wait for DQ3 to go high */
	do {
		status = readmem(memory);
	} while ((status & mask3) != mask3);
#endif
...
__SRCVERSION("$URL: http://svn/product/branches/6.5.0/trunk/hardware/flash/mtd-flash/amd/a29f100_v2erase.c $ $Rev: 710521 $")
#endif
