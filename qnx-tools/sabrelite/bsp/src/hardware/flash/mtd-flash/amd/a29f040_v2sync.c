 ...
 * Description
 *
 * This is the only MTDv2 sync callout for AMD flash.
 */

int f3s_a29f040_v2sync(f3s_dbase_t *dbase,
                       f3s_access_t *access,
                       uint32_t flags,
                       uint32_t offset)
{
	...
	/* MXIC workaround: issue 8 commands within 30us */
	if (dbase->flags & 0x10000000) {
		uintptr_t amd_cmd1;
		uintptr_t amd_cmd2;
		...
		send_command(memory, 0x33);
		send_command(memory, 0xF0);
		InterruptEnable();
	}
	return (EOK);
}
...
__SRCVERSION("$URL: http://svn/product/branches/6.5.0/trunk/hardware/flash/mtd-flash/amd/a29f040_v2sync.c $ $Rev: 710521 $")
#endif
