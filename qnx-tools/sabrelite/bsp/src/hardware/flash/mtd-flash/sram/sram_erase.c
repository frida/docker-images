 ...
 * Description
 *
 * This is the only MTDv1 erase callout for RAM / SRAM. The extra code
...
void f3s_sram_erase(f3s_dbase_t *dbase,
                    f3s_access_t *access,
                    uint32_t flags,
                    uint32_t offset,
                    int32_t size)
				...
				if (rand() < ((FAULT_DENSITY * RAND_MAX) / 1000)) {
					...
					if ((show & SHOW_FAULT) && (erases >= silent)) {
						fprintf(stderr, "(devf  t%d::%s:%d) flash[%2X] = b%8s (%2X), good = b11111111 (FF), bad = b%8s (%2X)\n",
						        pthread_self(), __func__, __LINE__, i, ltoa (memory[i], buf1, 2), memory[i],
						           ltoa (bad,       buf2, 2), bad);
					}
					memory[i] = bad;
...
__SRCVERSION("$URL: http://svn/product/branches/6.5.0/trunk/hardware/flash/mtd-flash/sram/sram_erase.c $ $Rev: 710521 $")
#endif
